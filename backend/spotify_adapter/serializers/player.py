...
class DeviceSerializer(serializers.Serializer):
    ...
    type = serializers.CharField(
        help_text=_('Device type, such as "computer", "smartphone" or "speaker". ')
    )
...
class PlayerContextSerializer(serializers.Serializer):
    type = serializers.CharField(
        help_text=_('The object type, e.g. "artist", "playlist", "album", "show". ')
    )
    ...
    uri = serializers.CharField(help_text=_("The Spotify URI for the context."))


class PlayerActionsSerializer(serializers.Serializer):
    interrupting_playback = serializers.BooleanField(
        required=False, help_text=_("Interrupting playback. Optional field.")
    )
    pausing = serializers.BooleanField(
        required=False, help_text=_("Pausing. Optional field.")
    )
    resuming = serializers.BooleanField(
        required=False, help_text=_("Resuming. Optional field.")
    )
    seeking = serializers.BooleanField(
        required=False, help_text=_("Seeking playback location. Optional field.")
    )
    skipping_next = serializers.BooleanField(
        required=False, help_text=_("Skipping to the next context. Optional field.")
    )
    skipping_prev = serializers.BooleanField(
        required=False, help_text=_("Skipping to the previous context. Optional field.")
    )
    toggling_repeat_context = serializers.BooleanField(
        required=False, help_text=_("Toggling repeat context flag. Optional field.")
    )
    toggling_shuffle = serializers.BooleanField(
        required=False, help_text=_("Toggling shuffle flag. Optional field.")
    )
    toggling_repeat_track = serializers.BooleanField(
        required=False, help_text=_("Toggling repeat track flag. Optional field.")
    )
...
class PlayerPlaybackStateSerializer(serializers.Serializer):
    device = DeviceSerializer(help_text=_("The device that is currently active."))
    repeat_state = serializers.ChoiceField(
        choices=["track", "context", "off"],
        help_text=_('The repeat state. Options are "track", "context" and "off".'),
    )
    shuffle_state = serializers.BooleanField(help_text=_("If shuffle is on or off."))
    context = PlayerContextSerializer(
        allow_null=True, help_text=_("A Context Object. Can be null.")
    )
    timestamp = serializers.IntegerField(
        help_text=_("Unix Millisecond Timestamp when data was fetched.")
    )
...
class PlayHistorySerializer(serializers.Serializer):
    track = TrackSerializer(help_text=_("The track the user listened to."))
...
class RecentlyPlayedTracksSerializer(serializers.Serializer):
    ...
    total = serializers.IntegerField(
        min_value=0, help_text=_("The total number of items available to return.")
    )
